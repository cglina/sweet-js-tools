 ...
 * isStructuredJSON("hello") // false
 */
export function isStructuredJSON(input: string): boolean {
 ...
 */
export function isCSVString(input: string, strictColumns: boolean = false): boolean {
    const rows = input
        .trim()
        .split("\n")
        .map(r => r.trim())
    ...
    if (headerColumnCount < 2) return false
 ...
 */
export function stringDataCheck(input: string): StringDataFormats {
    const trimmed = input.trim()

    if (trimmed.length === 0) return "unknown"

    if (isStructuredJSON(trimmed)) return "json"
    if (isCSVString(trimmed)) return "csv"
...
export function parseData(data: any): any {
    ...
    try {
        ...
        if (dataType === "csv") return csvObjectify(data)
