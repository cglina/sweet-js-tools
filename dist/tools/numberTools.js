 ...
 */
export function absValueFloor(nr = 0) {
 ...
 */
export function absValueCeil(nr = 0) {
    if (!Number.isFinite(nr))
        return 0;
    return nr < 0
        ? Math.floor(nr)
 ...
 * randomNr(NaN) → 0
 */
export function randomNr(upperLimit = 5) {
 ...
 * randomNr0(NaN) → 0
 */
export function randomNr0(upperLimit = 10) {
    if (!Number.isFinite(upperLimit))
        return 0;
    const limit = absValueFloor(upperLimit);
    if (limit === 0)
 ...
 */
export function randomInRange(firstNr = 2, secondNr = 8) {
    if (!Number.isFinite(firstNr) || !Number.isFinite(secondNr))
        return 0;
 ...
 * randomUpTo(4, 4) → 4
 */
export function randomUpTo(firstNr = 2, secondNr = 8) {
 ...
 */
export function randomBetween(firstNr = 2, secondNr = 8) {
    const a = Number.isFinite(firstNr) ? absValueFloor(firstNr) : 0;
    const b = Number.isFinite(secondNr) ? absValueFloor(secondNr) : 0;
    const lowNr = Math.min(a, b);
    const highNr = Math.max(a, b);
 ...
 * floorDecimal(-5.369124, 2) → -5.36
 */
export function floorDecimal(nr = 1, decimalPlaces = 2) {
    if (!Number.isFinite(nr))
        return 0;
 ...
 * ceilDecimal(-5.363124, 2) → -5.37
 */
export function ceilDecimal(nr = 1, decimalPlaces = 2) {
    if (!Number.isFinite(nr))
        return 0;
    const places = Number.isFinite(decimalPlaces)
        ? Math.min(14, Math.abs(Math.floor(decimalPlaces)))
 ...
 */
export function roundDecimal(nr = 1, decimalPlaces = 2) {
    if (!Number.isFinite(nr))
        return 0;
    const places = Number.isFinite(decimalPlaces)
        ? Math.min(14, Math.abs(Math.floor(decimalPlaces)))
 ...
 */
export function isOdd(nr = 2) {
    if (!Number.isFinite(nr) || !Number.isInteger(nr))
        return false;
 ...
 */
export function clamp(nr = 0, min = 0, max = 1) {
 ...
 * findFactors(0) → []
 */
export function findFactors(nr = 1) {
    if (!Number.isFinite(nr))
        return [];
 ...
 */
export function findLowestCommonFactor(nr1 = 2, nr2 = 4) {
    if (!Number.isFinite(nr1) || !Number.isFinite(nr2))
        return 0;
