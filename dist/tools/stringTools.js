 ...
 * // "Hello. Nice."
 */
export function capitalizeText(text, punctuation = [".", "!", "?"], enforceString = false) {
    const stops = Array.isArray(punctuation) ? punctuation.join("") : punctuation;
    const regEx = new RegExp(`([${escapeRegex(stops)}]+\\s+)(\\S)`, "g");
    function upperAfterStop(_match, p1, p2) {
        return p1 + p2.toUpperCase();
    }
    function capitalizeSentences(par) {
 ...
 * // [2, 2]
 */
export function wordCount(text, mode = "chunk") {
    const regEx = mode === "word"
        ? regWord
        : mode === "split"
            ? regSplitWord
            : regWordChunk;
 ...
 * // "hello world\nnice day"
 */
export function trimExtra(text = "") {
    return text
        .trim()
        .replace(/[ \t]+/g, " ")
        .replace(/\s*\n+\s*/g, "\n");
 ...
 *
 */
export function normalizeString(str, cleanSpacing = true) {
...
export function normalizeText(text) {
    if (Array.isArray(text))
        return text.map((item) => normalizeString(item));
 ...
 * // ["hi_", "a_b"]
 */
export function replaceSymbols(text, replacement = " ") {
    function replaceSym(str) {
        return str.replace(/[^\p{L}\d\s]/gu, replacement);
    }
    if (Array.isArray(text))
        return text.map(replaceSym);
 ...
 * // "crème_brûlée!"
 */
export function toSnakeCase(text, normalizeInput = true) {
 ...
 * // "hello-world-test"
 */
export function toKebabCase(text, normalizeInput = true) {
    function convert(str) {
        const result = normalizeInput ? normalizeString(str) : str.trim();
        return result
            // split camelCase
